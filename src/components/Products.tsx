      ...
      <StaggerGroup className="grid gap-8 lg:grid-cols-3">
        {featuredItems.map((item) => (
          <StaggerItem key={item.title} className="h-full">
            <Link
              href={item.href}
              className="group relative block h-full overflow-hidden rounded-[36px] border border-white/10 bg-white/[0.03] p-10 backdrop-blur-md transition-all duration-500 hover:-translate-y-2 hover:border-[var(--brand-blue)] hover:shadow-[0_0_50px_rgba(14,108,178,0.18)]"
            >
